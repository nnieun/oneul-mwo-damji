...
def test_complete_demo_and_swagger_contract(tmp_path):
    with TestClient(create_app(Database(tmp_path/'integration.db'),camera=Camera(demo=True))) as c:
        spec=c.get('/openapi.json').json()
        expected={
            '/api/health':['get'],'/api/products':['get'],
            '/api/camera/status':['get'],'/api/camera/start':['post'],'/api/camera/stop':['post'],'/api/camera/stream':['get'],
            '/api/recognition/scan':['post'],'/api/recognition/candidates':['post'],'/api/recognition/candidates/{candidate_id}/dismiss':['post'],
        ...
        scan=c.post('/api/recognition/scan').json()
...
def test_model_specific_label_mapping_and_candidate_rollback(tmp_path,monkeypatch):
    ...
    with TestClient(create_app(Database(tmp_path/'mapping.db'),camera=Camera(demo=True))) as c:
        cid=c.post('/api/carts').json()['id']
