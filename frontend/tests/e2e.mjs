...
try {
  ...
  await page.getByText('320원 →',{exact:true}).waitFor()
  ...
  await page.getByRole('button',{name:'계란 (1개) 수량 늘리기',exact:true}).click()
  await page.getByText('4,940원',{exact:true}).waitFor()
  await page.getByRole('button',{name:'소금 (500g) 삭제',exact:true}).click()
  await page.getByText('3,140원',{exact:true}).waitFor()
